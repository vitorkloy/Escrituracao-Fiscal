...
export interface CteXmlSalvoInfo {
  chave: string
  caminho: string
  ano: string
  mes: string
...
export function listarXmlsCteSalvos(
  pastaRaiz: string,
  cnpj14: string,
  ...
  for (const anoEnt of fs.readdirSync(base, { withFileTypes: true })) {
    ...
    for (const mesEnt of fs.readdirSync(anoPath, { withFileTypes: true })) {
      ...
      for (const arq of fs.readdirSync(mesPath, { withFileTypes: true })) {
        ...
        const procResOut = baseNome.match(/^(\d{44})_(procCTe|resCTe|outro)$/)
        const eventoComSufixo = baseNome.match(/^(\d{44})_evento(?:_|$)/)
        const nsuSemChave = baseNome.match(/^NSU_(\d+)_/)
        const chave =
          procResOut?.[1] ??
          eventoComSufixo?.[1] ??
          (/^\d{44}$/.test(baseNome) ? baseNome : nsuSemChave ? `NSU_${nsuSemChave[1]}` : baseNome)
