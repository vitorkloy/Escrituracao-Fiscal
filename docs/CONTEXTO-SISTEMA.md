...
- **Emitente vs destinatário (NT 2014.002):** DistDFe **não** entrega `procNFe` ao emitente (`consChNFe` → **641**). Na fila, **Autorização de Uso** = documento `resNFe` (`cSitNFe` 1=autorizada, 2=cancelada, 3=denegada) — **não** é “evento”. Eventos = `resEvento` / `_evento_` (ciência, CC-e, cancelamento). Fluxo: DistDFe → chaves → tentativa `NFeConsultaProtocolo4` SP → se só protocolo, **Portal Nacional** (`nfe:portal-baixar`, mesma técnica do FSist: consulta completa + captcha + mTLS do A1 no Windows) → fallback Importar saída (ERP).
