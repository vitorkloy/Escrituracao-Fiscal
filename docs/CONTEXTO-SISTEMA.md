...
- **Sincronização por NSU:** pasta raiz configurável; subpastas por CNPJ; estado `.nfe-dist-state.json`; `sync-debug.log`; XMLs em `{ano}/{mes}/` com sufixos `_procNFe`, `_resNFe`, `_evento`, `_outro` para não colidir nota/evento na mesma chave; fallback `sem-data/00` quando não há data.
